    ...
    protected function makeTableRows($model)
    {
        ...
        $editAndDelete = <<<EOT
                    <td>
                        ...
                        {{ Form::close() }}
                        {{ link_to_route('{$models}.edit', 'Edit', array(\${$model}->id), array('class' => 'mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent')) }}
                    </td>
EOT;
    ...
    public function makeFormElements()
    {
        ...
        foreach($this->cache->getFields() as $name => $type)
        {
            ...
            {
                case 'integer':
                   $element = "{{ Form::input('number', '$name', Input::old('$name'), array('class'=>'mdl-textfield__input')) }}";
                    break;

                case 'text':
                    $element = "{{ Form::textarea('$name', Input::old('$name'), array('class'=>'mdl-textfield__input')) }}";
                    break;
                ...
                default:
                    $element = "{{ Form::text('$name', Input::old('$name'), array('class'=>'mdl-textfield__input')) }}";
                    break;
            }
            ...
            $element
            {{ Form::label('$name', '$formalName:', array('class'=>'mdl-textfield__label')) }}
            </div>
